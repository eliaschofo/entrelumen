...
Projects support optional reward (item ID), granted once after successful consuming delivery. Optional `extraRewards` (item ID to count, 1..64, not repeating `reward`) are granted with it; since 24 September 2026 `first_signal` uses it for the first two calibration frames, which have no crafting recipe. Module projects consume materials and grant their module; controller checks placed modules. Acts2-6 costs are development balancing data, not final kitchen-sink integration. Prerequisites field is requires (project IDs).
