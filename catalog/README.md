...
`catalog/families/*.json` holds separately reviewed roles and exact file pins: CurseForge project/file IDs, or `"provider": "modrinth"` with the project/version IDs, the official CDN URL and the version API's SHA-1 and SHA-512. A CurseForge pin may also carry the SHA-512 of the same bytes published on Modrinth. `--add-families` retains every existing lock entry and local origin, resolves only missing selected mods and dependencies, and validates both sides before writing. A family may replace an earlier entry only by naming it in `replaces` (the old filename, the pinned file that replaces it and why); `--check` then rejects a lock that still holds the old file. Library-only JARs with no top-level `[[mods]]` (Kotlin for Forge) resolve through the mods they bundle. A changed family pin or missing dependency rejects the addition. `--check` also verifies these pins. Full `--refresh` remains an explicit rebuild and can update unpinned entries; use additive curation for ordinary integration.
...
- AE2 is the principal digital-storage system; Refined Storage is an optional alternative at the same act and cost (Elias, mod ping-pong round 2). Expedition dimensions: Aether (with Deep Aether), Twilight Forest and Bumblezone, plus the Undergarden (III), Eternal Starlight (IV) and Ad Astra's planets (V), each opened by an act component in its portal or launch recipe. No extra biome overhaul stack. RFTools Power stores energy, not items; RFTools Storage and Dimensions are excluded.
...
| [Mod ping-pong, rounds 1-3](../docs/design/mod-pingpong.md#resultado-de-la-integración-rondas-1-a-3) | 29 (15 from Modrinth, 1 from the CurseForge CDN) / 4 | 308 client / 266 server | [mod ping-pong](../docs/verification/mod-pingpong-runtime.json) |
| [Mod ping-pong, round 4](../docs/design/mod-pingpong.md#resultado-de-la-integración-ronda-4) | 5 (all from Modrinth) / 1 new (Kotlin for Forge) and Create: Dragons Plus 1.11.7b replaced by 1.11.9, declared in the family's `replaces` | 314 client / 272 server | [round 4](../docs/verification/mod-pingpong-r4-runtime.json) |

Decisions against Oritech, Ender IO, Modern Industrialization, Extreme Reactors, Applied Flux, Productive Trees and Super Factory Manager are recorded in the industrial note; Oritech, Ender IO and Modern Industrialization (as the fallback for GregTech, which has no working 1.21.1 server build) came in on 24 September at Elias's direction; the Apotheosis suite, first kept out in the arcane note, was added on 23 September at Elias's direction. The lock, companion and QA server stay on NeoForge 21.1.249; no loader change was required.
