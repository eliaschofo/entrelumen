"""Generate/check the 16 static integration recipes; never writes a live instance.

15 are shaped crafting recipes whose pattern draws the component (Elias's playtest of 24 September
...
def recipes_from(design):
    ...
    if len(projects) != 16:
        raise ValueError("Expected 16 integration projects")
...
def check_not_module(rid, output):
    """Ark v2: a module comes only from its act's Atlas project, never from a table recipe."""
    if output["id"] in ARK_MODULES:
        raise ValueError(f"Ark modules have no table recipe since Ark v2: {rid}")
