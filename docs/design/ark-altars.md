...
The repair's reference, off the server thread, is unchanged: 127 ms per terrain chunk and 33 ms per decorated chunk for the overworld noise generator in this run. TPS with players and the full pack has not been measured.
...
- **Python checks.** The Python steps of `.github/workflows/verify.yml`, `tools/check_guides.py` and `tools/curate_pack.py --check`: see the receipt `ci-checks-*.log`.
...
- 25 September: run `AltarFullpackGameTests` on the owned server after this branch (dye coverage over every registered biome, modded trees, ink caps, the repair against the pack's generator and the claims). Review the new status lines, tooltips, the Nature module's garden row and the Atlas guide in a client in EN and ES.
