...
Measured in the isolated GameTest server, with about 50 other tests running at the same time (two runs): the controller's 261,316-block city took 47–63 ticks, 0.9–1.6 s of placing in total, worst tick 29–65 ms (one cube can exceed the 15 ms budget under load), 8–12 s wall clock including the background read and chunk loading. A first version that placed 32 × 32 full-height columns and let the tick generate chunks spent 4.7 s in 7 ticks, with a 1.3 s worst tick.

**Current template.** `solsticio/city.nbt`, made by the controller with `art/solsticio/city6.py --export` (draft 6, 24 September; regenerate, never edit by hand). SHA-256 `dd9740c6614b2a264914e34e1479863e7e629ecc42bfab0a76cf76e34e5b3c63`, 250 × 189 × 239, DataVersion 3955, 268 palette states, 557,413 entries (blocks plus markers), no entities, no `provisional` marker. An organic island with relief: the symmetric civic core on the summit (Ayuntamiento rotunda under the armillary sun, the four NPC halls, the four player plots), then winding streets, a river with bridges, 181 buildings on levelled pads (16 shops, 4 inns, residents), plazas with fountains, lighthouses and three easter eggs. About 19,000 blocks come from the pack's decorative mods (Macaw's, Chipped, Handcrafted, Supplementaries, Refurbished); a server without those mods places air there. The acropolis floor is template layer 78, so with `BASE_Y` 64 it stands at world y 142. Placement takes about 23 s of spread-out work on the first visit, so the Solsticio GameTests allow 3,600 ticks. Drafts 2 to 5 stay in `art/solsticio/` for reference. The provisional 61 × 12 × 61 placeholder and its generator (`tools/build_solsticio_provisional.py`) remain only in commit `5081b7b`.
...
- Placed dormant on `town_hall_portal`. A team that passes the gate and reached act VI's accord right-clicks it with each of its relics (`entrelumen:heliodor_relic_1..3`, from missions 4, 6 and 8, consumed; operators get them with `/entrelumen admin solsticio relics`). With the team's three set, a right-click by a player carrying **their own** broken Light Key completes mission 10; the first time, it opens the portal. The key is only shown, not consumed — the story promises it as a way home "whenever you want"; consuming it is a one-line change if Elias prefers.
...
- ~~Missions that award the relics and the named villagers on their marker points~~: act VI ([act-six.md](act-six.md), 24 September 2026). Since then the portal counts relics per team: a team sets its own three after its accord (mission 9) and completes mission 10 with its own key; the first one opens the portal for everyone and liberates the Entrelumen, later teams set theirs in the open portal. The trading hall's natives, the shops and moved-villager discounts are in [solsticio-commerce.md](solsticio-commerce.md).
