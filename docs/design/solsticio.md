...
**Current template.** `solsticio/city.nbt`, made by the controller with `art/solsticio/city3.py --export` (draft 3, 24 September; regenerate, never edit by hand). SHA-256 `4d7c5eba5330516397fa7931e19b63d6402c5bb4f595d77365c79d71651e7103`, 127 × 186 × 127, DataVersion 3955, 81 palette states, 184,428 entries (blocks plus the 12 markers), no entities, no `provisional` marker. The island is a hollow shell: three terraces (acropolis, city ring, hanging gardens) on top, a stalactite belly with an amethyst root below, and an armillary sphere with the frozen sun above the rotunda. The acropolis floor is template layer 99, so with `BASE_Y` 64 it stands at world y 163 and the sphere tops out near y 249: arrival (0, 165, 6), portal (0, 165, 0). The five beacons (the dome lantern and four lighthouses) sit on hidden gold bases. `BASE_Y` stays at 64 on purpose: the whole island stays above the y 63 horizon line, where vanilla starts drawing the dark lower sky, and falls into the void are caught long before the void itself. Draft 2 (`city.py`, 105 × 78 × 105) stays in the repository for reference. The provisional 61 × 12 × 61 placeholder and its generator (`tools/build_solsticio_provisional.py`) remain only in commit `5081b7b`.
