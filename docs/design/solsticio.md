...
**Size.** Any size; the 48-block structure-block limit does not apply. The loader reads each piece's `size`, runs the vanilla structure datafixer from its `DataVersion`, and never assumes a maximum.
...
**Placement.** Once per world, the first time something needs the city: the Ark activation (key forging), a Light Key use, anyone entering the dimension, or `/entrelumen admin solsticio place`. Pieces are read and cut into 32 × 32 full-height column slices off the server thread; the server places at most ~24,576 blocks per tick (always at least one slice). Progress (`slicesDone`) is saved, so a restart resumes where it stopped; a changed template list during an interrupted placement restarts it. When done, `SolsticioData.placements` becomes 1 and it is never placed again: a template changed after a world placed the city needs an explicit migration, not a re-run.
...
**Plots.** The first member of a team who stands in the ready city claims one plot for the team's campaign (party id, or the player for a solo campaign), provided the team passes the gate. One plot per team; it keeps it while the campaign exists (archived parties included, so a recovered party finds its house). The provisional city has **4** symmetric plots; when all are taken, newcomers are told so and may still visit. An operator frees one with `/entrelumen admin solsticio plot release <index>` (blocks stay). A player who later joins a party keeps their solo plot under their personal campaign.
...
- In-game visual review on a client: sky, fog, motes, portal particles, the definitive city.
