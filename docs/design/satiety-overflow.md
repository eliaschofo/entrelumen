...
- **Before the bite:** `PlayerInteractEvent.RightClickBlock`, at the highest priority and also when cancelled, opens a *bite window* for that player. Until 24 September it ran last and skipped cancelled events; the full-pack QA showed why that missed bites: Amendments 2.1.9 (directional cakes, on by default) eats a vanilla cake from its own `RightClickBlock` handler, turns it into its directional cake and cancels the event. A window only measures food actually added, so a cancel that eats nothing (claim protection, for example) still converts nothing.
