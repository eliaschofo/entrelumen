...
- **Instalación:** en una instancia ya jugada los mods ya escribieron `config/ironjetpacks/jetpacks/*.json`, `config/oritech-startup.toml` y `config/pneumaticcraft-common.toml` con los valores de fábrica, y `sync_pack.py` los frena como cambio local; hay que adoptarlos con backup, como se hizo con `ftbultimine-server.snbt`. En una instalación nueva el pack llega antes del primer arranque y no hace falta nada. Iron Jetpacks manda sus tipos desde el servidor; Oritech y PneumaticCraft leen su archivo local, así que el cliente también tiene que recibir el pack.
