...
Minecraft 1.21.1/NeoForge's block respawn extension, `DismountHelper`, cancelable spawn setter, `PlayerRespawnPositionEvent` and post-respawn player event implement the behavior. The current original BookViewScreen journal receives bilingual instructions and personal status. Rendered acceptance remains pending while Computer Use is disabled.
