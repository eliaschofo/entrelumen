...
Decision, 2026-09-23. Implemented with isolated headless evidence. The full-pack claim case is written and registered, but has not run and awaits integration. Client review is pending too. This adds a practical benefit to the existing Nature module. It does not change campaign deliveries or grant rewards.
...
- Full-pack, pending integration: `NatureRestorationGameTests.natureRestorationRespectsForeignFtbChunksClaim`. A real `/ftbchunks claim` by one team must refuse another team's restoration inside the claimed chunk without charge, while the owner can restore it. It is compiled into the QA JAR and registered in `FullpackQABootstrap`, but not run: the QA server was not started for this change.

The isolated fixture is a flat plains world with a forest biome filled in. It uses mock connections and cancels a test listener in place of FTB Chunks; it does not prove full-pack compatibility. With these tests, the suite's shutdown world save took 8–12 minutes, against about 1 minute in the logistics receipt. That was measured while another worker's QA server loaded the same disk and has not been investigated.

Pending: the full-pack claim run and the Ars Nouveau and Nature's Aura coexistence check in the installed pack. Also rendered EN/ES review of chat, tooltip, journal and quest, a survival playtest on real mined and logged terrain, and modded-biome samples. Performance is bounded per use (at most 197 columns, 64 bone meal and 16 simulated trees, with no ticking), but TPS has not been measured.
