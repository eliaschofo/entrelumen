...
## Rehecho visual — 23-09-2026

Elias liberó todos los assets creativos. Se retiraron la ilustración filtrada (`pixel_filter.py`, `pixel-master.png`) y el Atlas gigante del título. Título: escena nativa PixelLab 384×216 (×5 nearest). Cargas: escena nativa 352×199 (recorte del borde blanco que devolvió el proveedor). Logo: wordmark de cobre en bloque 255×33 a 1:1 GUI, anclado mid-left x12, terminando 50 unidades sobre el centro; el generador ahora admite logos de hasta 256×64. Los cuatro accesos pequeños pasan a top-right (x −100 + 24·i, y 4) para no pisar el pie de versión ni el copyright. Botones: toba con marco de cobre; hover verdín con remaches turquesa; contraste de etiqueta ≥4,5:1. Se eliminaron texturas GUI sin uso (`icon_*`, `tile_*`) y la copia del Atlas en `fancymenu/assets`. El parser real de FancyMenu 3.9.12 aceptó los ocho layouts (`--verify-parser` con el JDK 21 de G:). Falta observarlo en el cliente: escalas GUI 2–4, EN/ES, foco de teclado y las pantallas de carga.

Ajuste del mismo día: Elias prefirió la ilustración original del título y el logo anterior. La ilustración del 12/9 pasó por PixelLab `/image-to-pixelart-pro` (243×139, 51 colores, recibo en `art/candidates/pixellab-v2/pp_pixelish/`) y es el fondo activo del título (×8 nearest), con el logo clásico 180×64 (`art/menu/logo-classic.png`) centrado sobre la columna como antes. La escena del observatorio (`scene-observatory.png`) y el wordmark de cobre quedan como alternativas y alimentan la portada de CurseForge. La pantalla de carga no cambia. Una primera conversión pedida como "pantalla de título" inventó logo y botones falsos y se descartó.
