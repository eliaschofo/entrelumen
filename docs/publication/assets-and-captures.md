# Publication staging kit — not published
...
## Real screenshot slots — intentionally empty

Do not upload blank placeholders or substitute generated scenery. These rows are a capture checklist, not assets or completed evidence.

| Slot | Required real capture | Caption EN / ES |
|---|---|---|
| Opening chapter | In-game FTB overview with readable authored routes; record locale and build. | A place to return: the opening chapter. / Un lugar al que volver: el primer capítulo. |
| Working kitchen | Actual survival kitchen serving the opening delivery. | Prepare the next expedition at home. / Prepará la próxima expedición en casa. |
| Atlas tools | Actual in-game inventory and held items, default resource pack. | Instruments for a forgotten horizon. / Instrumentos para un horizonte olvidado. |
| Team campaign | Real two-team session showing separate progress; redact player identity if needed. | A shared home, a team campaign. / Un hogar compartido, una campaña por equipo. |
| Ark | Actual functional build after commissioning is implemented and tested. | Connect six disciplines. / Conectá seis disciplinas. |

For each capture, retain: original PNG, build/lock revision, world snapshot, locale, actual graphics preset, capture date and explanation of what was tested. Record shaders and edits explicitly. Do not attach an FPS claim to an isolated screenshot; link the real benchmark evidence when available.

## Pre-submission content review

- Replace development counts and claims only when source and gameplay evidence justify them.
- Upload the companion dependency before the pack submission; keep release/download controls with the actual platform files.
- Confirm GitHub destination and license links before submitting. No credential or account access was attempted by this staging task.
- Use the same English-first/Spanish-second order, conceptual-art labels and development status in the platform description.
