...
`act_five.json` adds 24 original EN/ES quests, including four authoritative campaign mirrors. Total: 128 quests and 28 campaign tasks. Full generated chapter, locale and task-map fingerprints freeze the preceding 104 quests' IDs, text, layout and behavior. No previous chapter source changes.

The three prototype milestones (`resilient_backbone`, `renewal_engine`, `settlement_supply`) each depend directly on `atlas_voices`. Their server deliveries consume exactly one `entrelumen:ark_bus`, `entrelumen:renewal_engine` and `entrelumen:habitation_contract`, respectively. The `world_network` closure requires all three and consumes three paper plus one copper ingot, with no item reward. Act VI advances manually through the Atlas. No inventory tutorial or optional checkmark gates an authoritative milestone; gifts and early unrestricted crafting remain valid, while only explicit Atlas delivery advances the campaign. Installed equipment is not consumed.

The three static integration recipes use seven, eight and seven crafting slots. The habitation recipe's two fish stews and two mixed salads return four bowls through ordinary crafting remainders. Item tasks show non-consuming possession. Four optional, self-reported field notes cover buffer recovery, bounded renewal, an inhabitable base and an optional ComputerCraft status panel; the text does not claim automatic validation of these demonstrations. Pinned native recipe audit supplied the alloy, PCB, sky ingot, imperium, stew and salad paths. JEI and the native books remain guides to their earlier ingredients and working equipment.

`python tools/test_generate_quests.py` passes 21 focused contracts. `python tools/generate_quests.py --check` confirms five chapters, 128 quests, global IDs/DAG, geometry, locale parity and generated-file drift. These checks do not establish loaded recipes, survival acquisition, rendered quest layout or Atlas runtime delivery; those belong to integrated game validation.
