...
The other four modules have distinct read-only team journals. Their new targeted full-pack check preserves campaign, inventory and blocks; their completed states rendered in both client languages. Native chat hides the beginning of longer entries until its history is opened, so presentation remains provisional. The [first-hour survival protocol](docs/verification/first-hour-playtest.md) is ready, but the natural-acquisition playtest has not been run.
...
Los otros cuatro módulos tienen diarios distintos que leen la campaña actual del equipo. Su nueva prueba con todos los mods conservó campaña, inventario y bloques; los estados completados se vieron en ambos idiomas. Los textos largos requieren abrir el historial del chat para leer el principio, por lo que esa presentación sigue provisional. Está listo el [protocolo de supervivencia de la primera hora](docs/verification/first-hour-playtest.md), pero todavía falta ejecutar ese recorrido consiguiendo los materiales al jugar.
